...
class BlocknetManager:
    ...
    def setup(self):
        ...
        width = 350
        self.blocknet_core_label = ctk.CTkLabel(self.title_frame,
                                                text=widgets_strings.blocknet_frame_title_string,
                                                width=width,
                                                anchor="w")

        # Label for Data Path
        self.blocknet_data_path_label = ctk.CTkLabel(self.title_frame, text="Data Path: ")

        width = 343
        self.blocknet_data_path_entry_string_var = ctk.StringVar(value=self.utility.data_folder)
        self.blocknet_data_path_entry = ctk.CTkEntry(self.title_frame,
        ...
        self.blocknet_custom_path_button = ctk.CTkButton(self.title_frame,
        ...
        self.blocknet_download_bootstrap_button = ctk.CTkButton(self.title_frame,
                                                                image=self.parent.transparent_img,
        ...
        self.blocknet_data_path_status_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.blocknet_process_status_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.blocknet_conf_status_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.blocknet_rpc_connection_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
    ...
    def download_bootstrap_command(self):
        disable_button(self.blocknet_download_bootstrap_button, img=self.parent.install_greyed_img)
        self.bootstrap_thread = Thread(target=self.utility.download_bootstrap, daemon=True)
        self.bootstrap_thread.start()

    def open_custom_path_dialog(self):
        from conf_data import blocknet_default_paths
        expanded_path = None
        print(f"custom_path: {self.parent.custom_path}")

        if self.parent.custom_path is None:
            ...
            initialdir = self.parent.custom_path
    ...
    def on_custom_path_set(self, custom_path):
        self.utility.set_custom_data_path(custom_path)
        self.blocknet_data_path_entry_string_var.set(self.utility.data_folder)
    ...
    def update_blocknet_bootstrap_button(self):
        bootstrap_download_in_progress = bool(self.utility.bootstrap_checking)
        enabled = (self.utility.data_folder and not bootstrap_download_in_progress and
                   not self.blocknet_process_running)
        if enabled:
            enable_button(self.blocknet_download_bootstrap_button, img=self.parent.install_img)
        else:
            disable_button(self.blocknet_download_bootstrap_button, img=self.parent.install_greyed_img)
        if bootstrap_download_in_progress:
            if self.utility.bootstrap_percent_download:
                var = f"{self.utility.bootstrap_percent_download:.1f}%"
            elif self.utility.bootstrap_extracting:
                var = "Unpacking"
            else:
                var = "Loading"
        else:
    ...
    def update_blocknet_process_status_checkbox(self):
        ...
        var = widgets_strings.blocknet_running_string if self.blocknet_process_running else widgets_strings.blocknet_not_running_string
    ...
    def update_blocknet_custom_path_button(self):
        ...
        bootstrap_download_in_progress = (
                self.utility.bootstrap_checking or self.utility.bootstrap_percent_download)
    ...
    def update_blocknet_conf_status_checkbox(self):
        ...
        conf_exist_and_parsed = bool(
            self.utility.blocknet_conf_local and self.utility.xbridge_conf_local)
    ...
    def update_blocknet_data_path_status_checkbox(self):
        ...
        exist = self.utility.check_data_folder_existence()
    ...
    def update_blocknet_rpc_connection_checkbox(self):
        ...
        self.blocknet_rpc_connection_checkbox_state.set(self.utility.valid_rpc)

        # blocknet_rpc_connection_checkbox_string_var
        var = widgets_strings.blocknet_active_rpc_string if self.utility.valid_rpc else widgets_strings.blocknet_inactive_rpc_string
...
class BinaryManager:
    def __init__(self, parent, master_frame, title_frame):
        ...
        self.bin_blocknet_start_close_button_string_var = None
        self.bin_blocknet_start_close_button = None
    ...
    def setup(self):
        ...
        self.bins_header_label = ctk.CTkLabel(self.title_frame,
                                              text="Binaries Control panel:")
        self.title_frame.columnconfigure(1, weight=1)

        self.bins_found_label = ctk.CTkLabel(self.title_frame,
                                             text="Found:",
                                             anchor='s')

        self.bins_button_switch_theme = ctk.CTkButton(self.title_frame,
                                                      image=self.parent.theme_img,
                                                      command=self.parent.switch_theme_command,
                                                      text='',
        ...
        # Creating labels
        self.bins_blocknet_label = ctk.CTkLabel(self.master_frame, text="Blocknet Core:")
        self.bins_blockdx_label = ctk.CTkLabel(self.master_frame, text="Block-DX:")
        self.bins_xlite_label = ctk.CTkLabel(self.master_frame, text="Xlite:")
        ...
        self.bins_blocknet_version_optionmenu = ctk.CTkOptionMenu(self.master_frame,
                                                                  values=self.parent.blocknet_manager.blocknet_version,
                                                                  state='disabled')
        self.bins_blockdx_version_optionmenu = ctk.CTkOptionMenu(self.master_frame,
                                                                 values=self.parent.blockdx_manager.blockdx_version,
                                                                 state='disabled')
        self.bins_xlite_version_optionmenu = ctk.CTkOptionMenu(self.master_frame,
                                                               values=self.parent.xlite_version,
                                                               state='disabled')
        self.bins_blocknet_found_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
                                                                       text='',
                                                                       variable=self.blocknet_bin_installed_boolvar,
                                                                       state='disabled',
                                                                       corner_radius=25, width=1)
        self.bins_blockdx_found_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
                                                                      text='',
                                                                      variable=self.parent.binary_manager.blockdx_bin_installed_boolvar,
                                                                      state='disabled',
                                                                      corner_radius=25)
        self.bins_xlite_found_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
                                                                    text='',
        ...
        self.bins_install_delete_blocknet_button = ctk.CTkButton(self.master_frame,
                                                                 state='normal', image=self.parent.transparent_img,
        ...
        self.bins_install_delete_blockdx_button = ctk.CTkButton(self.master_frame,
                                                                state='normal', image=self.parent.transparent_img,
        ...
        self.bins_install_delete_xlite_button = ctk.CTkButton(self.master_frame,
                                                              state='normal',
                                                              image=self.parent.transparent_img,
                                                              ...
                                                              # text="",
                                                              corner_radius=25)
        self.bin_blocknet_start_close_button_string_var = ctk.StringVar(value='')
        self.bin_blocknet_start_close_button = ctk.CTkButton(self.master_frame,
                                                             image=self.parent.transparent_img,
    ...
    def start_or_close_xlite(self):
        ...
        if self.parent.xlite_manager.xlite_process_running:
            ...
            disable_button(self.xlite_start_close_button, img=self.parent.start_greyed_img)
    ...
    def start_or_close_blocknet(self):
        ...
        disable_button(self.bin_blocknet_start_close_button, img=img)
    ...
    def download_blocknet_command(self):
        disable_button(self.bins_install_delete_blocknet_button, img=self.parent.install_greyed_img)
    ...
    def download_blockdx_command(self):
        disable_button(self.bins_install_delete_blockdx_button, img=self.parent.install_greyed_img)
    ...
    def download_xlite_command(self):
        disable_button(self.bins_install_delete_xlite_button, img=self.parent.install_greyed_img)
    ...
    def update_blocknet_start_close_button(self):
        var = widgets_strings.close_string if self.parent.blocknet_manager.blocknet_process_running else widgets_strings.start_string
        self.bin_blocknet_start_close_button_string_var.set(var)

        if self.parent.blocknet_manager.blocknet_process_running:
            configure_tooltip_text(self.blocknet_start_close_button_tooltip, widgets_strings.close_string)
        else:
            configure_tooltip_text(self.blocknet_start_close_button_tooltip, widgets_strings.start_string)

        enabled = (not self.parent.blocknet_manager.utility.downloading_bin and
                   not self.disable_start_blocknet_button and
                   not self.parent.blocknet_manager.utility.bootstrap_checking)
        if enabled:
            img = self.parent.stop_img if self.parent.blocknet_manager.blocknet_process_running else self.parent.start_img
            enable_button(self.bin_blocknet_start_close_button, img=img)
        else:
            img = self.parent.stop_greyed_img if self.parent.blocknet_manager.blocknet_process_running else self.parent.start_greyed_img
            disable_button(self.bin_blocknet_start_close_button, img=img)
    ...
    def grid_bins_frame(self, x, y):
        ...
        self.bins_install_delete_xlite_button.grid(row=x + 3, column=y + 3, padx=padx_main_frame, pady=(2, 5),
                                                   sticky=button_sticky)
        padx_main_frame = (8, 8)
        self.bin_blocknet_start_close_button.grid(row=x + 1, column=y + 4, padx=padx_main_frame, sticky='e')
    ...
    def update_blockdx_start_close_button(self):
        ...
        var = widgets_strings.close_string if self.parent.blockdx_manager.blockdx_process_running else widgets_strings.start_string
        self.blockdx_start_close_button_string_var.set(var)

        enabled = (self.parent.blockdx_manager.blockdx_process_running or (
                not self.parent.blockdx_manager.utility.downloading_bin and
                self.parent.blocknet_manager.utility.valid_rpc) and
                   not self.disable_start_blockdx_button)
        if enabled:
            if self.parent.blockdx_manager.blockdx_process_running:
                configure_tooltip_text(self.blockdx_start_close_button_tooltip, msg=widgets_strings.close_string)
                img = self.parent.stop_img
            else:
                configure_tooltip_text(self.blockdx_start_close_button_tooltip, msg=widgets_strings.start_string)
                img = self.parent.start_img
            enable_button(self.blockdx_start_close_button, img=img)

            # self.blockdx_start_close_button_tooltip.hide()
        else:
            if self.parent.blockdx_manager.blockdx_process_running:
                img = self.parent.stop_greyed_img
                ...
                img = self.parent.start_greyed_img
            disable_button(self.blockdx_start_close_button, img=img)

    def update_xlite_start_close_button(self):
        # xlite_start_close_button_string_var
        var = widgets_strings.close_string if self.parent.xlite_manager.xlite_process_running else widgets_strings.start_string
        self.xlite_start_close_button_string_var.set(var)

        if self.parent.xlite_manager.xlite_process_running:
            configure_tooltip_text(self.xlite_start_close_button_tooltip, widgets_strings.close_string)
        else:
            configure_tooltip_text(self.xlite_start_close_button_tooltip, widgets_strings.start_string)

        # xlite_start_close_button
        disable_start_close_button = self.parent.xlite_manager.utility.downloading_bin or self.disable_start_xlite_button

        if not disable_start_close_button:
            img = self.parent.stop_img if self.parent.xlite_manager.xlite_process_running else self.parent.start_img
    ...
    async def coroutine_update_bins_buttons(self):
        ...
        percent_buff = self.parent.blocknet_manager.utility.binary_percent_download
        dl_string = f"{int(percent_buff)}%" if percent_buff else ""
        var_blocknet = dl_string if self.parent.blocknet_manager.utility.downloading_bin else ""
        ...
        if blocknet_boolvar:
            ...
            button_condition = self.parent.blocknet_manager.blocknet_process_running or self.parent.blocknet_manager.utility.downloading_bin
        else:
            configure_tooltip_text(self.bins_install_delete_blocknet_tooltip,
                                   global_variables.blocknet_release_url)
            button_condition = self.parent.blocknet_manager.utility.downloading_bin

        if button_condition:
            disable_button(self.bins_install_delete_blocknet_button,
                           img=self.parent.delete_greyed_img if blocknet_boolvar else self.parent.install_greyed_img)
        else:
            enable_button(self.bins_install_delete_blocknet_button,
                          img=self.parent.delete_img if blocknet_boolvar else self.parent.install_img)

        percent_buff = self.parent.blockdx_manager.utility.binary_percent_download
        dl_string = f"{int(percent_buff)}%" if percent_buff else ""
        var_blockdx = dl_string if self.parent.blockdx_manager.utility.downloading_bin else ""
        ...
        if blockdx_boolvar:
            ...
            button_condition = self.parent.blockdx_manager.blockdx_process_running or self.parent.blockdx_manager.utility.downloading_bin
        else:
            configure_tooltip_text(self.bins_install_delete_blockdx_tooltip,
                                   global_variables.blockdx_release_url)
            button_condition = self.parent.blockdx_manager.utility.downloading_bin

        if button_condition:
            disable_button(self.bins_install_delete_blockdx_button,
                           img=self.parent.delete_greyed_img if blockdx_boolvar else self.parent.install_greyed_img)
        else:
            enable_button(self.bins_install_delete_blockdx_button,
                          img=self.parent.delete_img if blockdx_boolvar else self.parent.install_img)

        percent_buff = self.parent.xlite_manager.utility.binary_percent_download
        dl_string = f"{int(percent_buff)}%" if percent_buff else ""
        var_xlite = dl_string if self.parent.xlite_manager.utility.downloading_bin else ""
        ...
        if xlite_boolvar:
            ...
            button_condition = self.parent.xlite_manager.xlite_process_running or self.parent.xlite_manager.utility.downloading_bin
        else:
            configure_tooltip_text(self.bins_install_delete_xlite_tooltip,
                                   global_variables.xlite_release_url)
            button_condition = self.parent.xlite_manager.utility.downloading_bin

        if button_condition:
            disable_button(self.bins_install_delete_xlite_button,
                           img=self.parent.delete_greyed_img if xlite_boolvar else self.parent.install_greyed_img)
        else:
            enable_button(self.bins_install_delete_xlite_button,
                          img=self.parent.delete_img if xlite_boolvar else self.parent.install_img)
...
class BlockDXManager:
    ...
    def setup(self):
        ...
        width = 540
        self.blockdx_label = ctk.CTkLabel(self.title_frame,
        ...
        self.blockdx_process_status_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.blockdx_valid_config_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
    ...
    def update_blockdx_process_status_checkbox(self):
        ...
        var = widgets_strings.blockdx_running_string if self.blockdx_process_running else widgets_strings.blockdx_not_running_string
    ...
    def update_blockdx_config_button_checkbox(self):
        ...
        valid_core_setup = bool(self.parent.blocknet_manager.utility.data_folder) and bool(
            self.parent.blocknet_manager.utility.blocknet_conf_local)
        if valid_core_setup and self.parent.blocknet_manager.utility.valid_rpc:
            var = widgets_strings.blockdx_valid_config_string if self.is_blockdx_config_sync else widgets_strings.blockdx_not_valid_config_string
            self.blockdx_valid_config_checkbox_string_var.set(var)
        else:
            self.blockdx_valid_config_checkbox_string_var.set(widgets_strings.blockdx_missing_blocknet_config_string)

        if valid_core_setup:
            xbridgeconfpath = os.path.join(self.parent.blocknet_manager.utility.data_folder, "xbridge.conf")
            rpc_user = self.parent.blocknet_manager.utility.blocknet_conf_local.get('global', {}).get('rpcuser')
            rpc_password = self.parent.blocknet_manager.utility.blocknet_conf_local.get('global', {}).get('rpcpassword')

            # blockdx_valid_config_checkbox_state
            blockdx_conf = self.utility.blockdx_conf_local
            ...
            self.blockdx_valid_config_checkbox_state.set(
                (self.is_blockdx_config_sync and self.parent.blocknet_manager.utility.valid_rpc))
...
class XliteManager:
    ...
    def setup(self):
        ...
        self.xlite_process_status_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.xlite_daemon_process_status_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.xlite_reverse_proxy_process_status_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.xlite_valid_config_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.xlite_daemon_valid_config_checkbox = ctkCheckBoxMod.CTkCheckBox(self.master_frame,
        ...
        self.xlite_store_password_button = ctk.CTkButton(self.title_frame,
    ...
    def xlite_store_password_button_mouse_click(self, event=None):
        ...
        if event and event.num == 1:
            ...
            if password:
                ...
                self.parent.xlite_password = password
            else:
    ...
    def update_xlite_process_status_checkbox(self):
        ...
        var = widgets_strings.xlite_running_string if self.xlite_process_running else widgets_strings.xlite_not_running_string
    ...
    def update_xlite_store_password_button(self):
        ...
        var = widgets_strings.xlite_stored_password_string if self.parent.xlite_password else widgets_strings.xlite_store_password_string
    ...
    def update_xlite_daemon_process_status(self):
        ...
        var = widgets_strings.xlite_daemon_running_string if self.xlite_daemon_process_running else widgets_strings.xlite_daemon_not_running_string
    ...
    def update_xlite_valid_config_checkbox(self):
        ...
        valid_config = True if self.utility.xlite_conf_local else False
    ...
    def update_xlite_daemon_valid_config_checkbox(self):
        ...
        valid_config = True if (self.utility.xlite_daemon_confs_local and
                                'master' in self.utility.xlite_daemon_confs_local) else False
...
class Blocknet_AIO_GUI(ctk.CTk):
    ...
    def setup_tooltips(self):
        ...
        CTkToolTip.CTkToolTip(self.bins_title_frame, message=widgets_strings.tooltip_bins_title_msg, delay=1,
                              follow=True,
                              bg_color=tooltip_bg_color, border_width=2, justify="left")
        CTkToolTip.CTkToolTip(self.binary_manager.bins_header_label, message=widgets_strings.tooltip_bins_title_msg,
                              delay=1,
                              follow=True,
                              bg_color=tooltip_bg_color, border_width=2, justify="left")
        CTkToolTip.CTkToolTip(self.xlite_manager.xlite_label, message=widgets_strings.tooltip_xlite_label_msg,
                              delay=1.0,
                              border_width=2, follow=True,
                              bg_color=tooltip_bg_color)
        CTkToolTip.CTkToolTip(self.binary_manager.bins_blocknet_label,
                              message=widgets_strings.tooltip_blocknet_core_label_msg, delay=1, follow=True,
                              bg_color=tooltip_bg_color,
                              border_width=2, justify="left")
        CTkToolTip.CTkToolTip(self.binary_manager.bins_blockdx_label, message=widgets_strings.tooltip_blockdx_label_msg,
                              delay=1,
                              follow=True,
                              bg_color=tooltip_bg_color, border_width=2, justify="left")
        CTkToolTip.CTkToolTip(self.binary_manager.bins_xlite_label, message=widgets_strings.tooltip_xlite_label_msg,
                              delay=1,
                              follow=True,
                              bg_color=tooltip_bg_color, border_width=2, justify="left")
        self.binary_manager.bins_install_delete_blocknet_tooltip = CTkToolTip.CTkToolTip(
            self.binary_manager.bins_install_delete_blocknet_button,
            message='', delay=1, width=1, follow=True,
            bg_color=tooltip_bg_color,
            border_width=2, justify="left")
        self.binary_manager.bins_install_delete_blockdx_tooltip = CTkToolTip.CTkToolTip(
            self.binary_manager.bins_install_delete_blockdx_button,
            message=global_variables.blockdx_release_url,
            delay=1, width=1, follow=True,
            bg_color=tooltip_bg_color,
            border_width=2, justify="left")
        self.binary_manager.bins_install_delete_xlite_tooltip = CTkToolTip.CTkToolTip(
            self.binary_manager.bins_install_delete_xlite_button,
            message=global_variables.xlite_release_url,
            delay=1, follow=True, bg_color=tooltip_bg_color,
            border_width=2, justify="left")
        self.binary_manager.blocknet_start_close_button_tooltip = CTkToolTip.CTkToolTip(
            self.binary_manager.bin_blocknet_start_close_button,
            delay=1, follow=True,
            bg_color=tooltip_bg_color,
            border_width=2, justify="left")
        self.binary_manager.blockdx_start_close_button_tooltip = CTkToolTip.CTkToolTip(
            self.binary_manager.blockdx_start_close_button,
            delay=1, follow=True,
            bg_color=tooltip_bg_color,
            border_width=2, justify="left")
        self.binary_manager.xlite_start_close_button_tooltip = CTkToolTip.CTkToolTip(
            self.binary_manager.xlite_start_close_button,
            delay=1, follow=True,
            bg_color=tooltip_bg_color,
            border_width=2, justify="left")
        CTkToolTip.CTkToolTip(self.blocknet_manager.blocknet_core_label,
                              message=widgets_strings.tooltip_blocknet_core_label_msg,
                              delay=1.0, border_width=2, follow=True, bg_color=tooltip_bg_color)

        CTkToolTip.CTkToolTip(self.blockdx_manager.blockdx_label,
    ...
    def init_grid(self):
        ...
        self.binary_manager.grid_bins_frame(x, y)
        self.blocknet_manager.grid_blocknet_frame(x, y, check_boxes_sticky)
        self.blockdx_manager.grid_blockdx_frame(x, y, check_boxes_sticky)
        self.xlite_manager.grid_xlite_frame(x, y, check_boxes_sticky)
    ...
    def update_status_gui(self):
        ...
        async def run_all_coroutines():
            await asyncio.gather(
                run_coroutine(self.blocknet_manager.coroutine_update_status_blocknet_core),
                run_coroutine(self.blockdx_manager.coroutine_update_status_blockdx),
                run_coroutine(self.xlite_manager.coroutine_update_status_xlite),
                run_coroutine(self.binary_manager.coroutine_update_bins_buttons)
            )
    ...
    async def coroutine_bins_check_aio_folder(self):
        ...
        self.binary_manager.blocknet_bin_installed_boolvar.set(blocknet_present)
        self.binary_manager.blockdx_bin_installed_boolvar.set(blockdx_present)
        self.binary_manager.xlite_bin_installed_boolvar.set(xlite_present)
        self.binary_manager.bins_last_aio_folder_check_time = time.time()
